// Modified version of ISAM2
// Needed for custom Dogleg optimization and for serialization
...
#include "calibration/backend/DoglegOptimizerImplCustom.h"
...
/* ************************************************************************* */
ISAM2Custom::ISAM2Custom(const ISAM2Params& params) : params_(params), update_count_(0) {
...
/* ************************************************************************* */
ISAM2Custom::ISAM2Custom() : update_count_(0) {
  if (params_.optimizationParams.type() == typeid(ISAM2DoglegParams))
    doglegDelta_ =
        boost::get<ISAM2DoglegParams>(params_.optimizationParams).initialDelta;
}
...
/* ************************************************************************* */
bool ISAM2Custom::equals(const ISAM2Custom& other, double tol) const {
...
/* ************************************************************************* */
GaussianFactorGraph ISAM2Custom::relinearizeAffectedFactors(
...
/* ************************************************************************* */
void ISAM2Custom::recalculate(const ISAM2UpdateParams& updateParams,
...
/* ************************************************************************* */
void ISAM2Custom::recalculateBatch(const ISAM2UpdateParams& updateParams,
  ...
  // std::cout << "Before eliminate" << std::endl;
  // auto elim_tree = GaussianEliminationTree(*linearized, affectedFactorsVarIndex, order);
  // std::cout << "Elim tree done" << std::endl;
  // auto isam2_junction_tree = ISAM2JunctionTree(elim_tree);
  // std::cout << "Junction tree done" << std::endl;
  // auto [bayesTree, factor_graph] = isam2_junction_tree.eliminate(params_.getEliminationFunction());
  // std::cout << "Bayes tree created" << std::endl;
  gttic(eliminate);
...
/* ************************************************************************* */
void ISAM2Custom::recalculateIncremental(const ISAM2UpdateParams& updateParams,
                                   const KeySet& relinKeys,
                                   const FastList<Key>& affectedKeys,
                                   KeySet* affectedKeysSet, Cliques* orphans,
                                   ISAM2Result* result) {
  gttic(recalculateIncremental);
  const bool debug = ISDEBUG("ISAM2Custom recalculate");
...
/* ************************************************************************* */
void ISAM2Custom::addVariables(const Values& newTheta,
                         ISAM2Result::DetailedResults* detail) {
  gttic(addNewVariables);

  theta_.insert(newTheta);
  if (ISDEBUG("ISAM2Custom AddVariables")) newTheta.print("The new variables are: ");
...
/* ************************************************************************* */
void ISAM2Custom::removeVariables(const KeySet& unusedKeys) {
...
/* ************************************************************************* */
ISAM2Result ISAM2Custom::update(
...
/* ************************************************************************* */
ISAM2Result ISAM2Custom::update(const NonlinearFactorGraph& newFactors,
                          const Values& newTheta,
                          const ISAM2UpdateParams& updateParams) {
  gttic(ISAM2_update);
  this->update_count_ += 1;
  // NOTE: Removed this for now to avoid need for ISAM2-Impl.h/.cpp
...
/* ************************************************************************* */
void ISAM2Custom::marginalizeLeaves(
      ...
      for (Key frontal : cg->frontals()) {
        ...
        if (!leafKeys.exists(frontal))
          throw std::runtime_error(
              ...
              "the ISAM2Custom object is now in an inconsistent state so should "
              "no longer be used.");
#endif
      }
    }
...
void ISAM2Custom::updateDelta(bool forceFullSolve) const {
  gttic(updateDelta);
  if (params_.optimizationParams.type() == typeid(ISAM2GaussNewtonParams)) {
    ...
    // NOTE: Modified to use custom version here!
    // Compute dogleg point
    DoglegOptimizerImpl::IterationResult doglegResult(
        DoglegOptimizerImplCustom::Iterate(
    ...
    throw std::runtime_error("iSAM2Custom: unknown ISAM2Params type");
  }
}

/* ************************************************************************* */
Values ISAM2Custom::calculateEstimate() const {
...
/* ************************************************************************* */
const Value& ISAM2Custom::calculateEstimate(Key key) const {
  const Vector& delta = getDelta()[key];
  return *theta_.at(key).retract_(delta);
}

/* ************************************************************************* */
Values ISAM2Custom::calculateBestEstimate() const {
  updateDelta(true);  // Force full solve when updating delta_
  return theta_.retract(delta_);
}

/* ************************************************************************* */
Matrix ISAM2Custom::marginalCovariance(Key key) const {
...
/* ************************************************************************* */
Matrix ISAM2Custom::marginalInformation(Key key) const {
  return marginalFactor(key, params_.getEliminationFunction())
      ->information();
}

/* ************************************************************************* */
const VectorValues& ISAM2Custom::getDelta() const {
  if (!deltaReplacedMask_.empty()) updateDelta();
  return delta_;
}

/* ************************************************************************* */
double ISAM2Custom::error(const VectorValues& x) const {
  return GaussianFactorGraph(*this).error(x);
}

/* ************************************************************************* */
VectorValues ISAM2Custom::gradientAtZero() const {
  // Create result
  VectorValues g;
